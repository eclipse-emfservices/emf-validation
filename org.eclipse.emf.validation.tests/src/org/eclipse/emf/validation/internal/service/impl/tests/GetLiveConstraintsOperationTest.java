...
public class GetLiveConstraintsOperationTest extends AbstractGetConstraintsOperationTest {
	...
	public void test_getNotification() {
		assertSame("Wrong notification", //$NON-NLS-1$
				notification, getLiveFixture().getNotification());
	}

	public void test_getEventType() {
		assertSame("Wrong event type", //$NON-NLS-1$
				EMFEventType.SET, getLiveFixture().getEventType());
	}

	public void test_getEObject() {
		assertSame("Wrong target object", //$NON-NLS-1$
				target, getFixture().getEObject());
	}
