...
public class CategoryTreeNode extends AbstractCategoryTreeNode {
	...
	private static class DelegateNode extends CategoryTreeNode {
		...
		public String toString() {
			...
			buf.append(NLS.bind(ValidationUIMessages.prefs_delegate_name, new Object[] { result }));
	 ...
	 * @param filter   a filter that defines which constraints I am interested in
	 */
	private CategoryTreeNode(CheckboxTreeViewer tree, Category category, ICategoryTreeNode parent,
	...
	/**
	 * Updates the GUI to reflect my current checked state, and propagates my state
	 * up the tree to my ancestors.
	 */
	...
	public void revertFromPreferences() {
		if (!hasDelegate()) {
			...
			for (Iterator<IConstraintNode> iter = getConstraints().iterator(); iter.hasNext();) {
