...
public class ValidationPreferencePage extends FieldEditorPreferencePage implements IWorkbenchPreferencePage {
	...
	protected void initialize() {
		...
		setCheckboxesEnablement(ValidationUIPlugin.getPlugin().getPreferenceStore()
	...
	private void createLiveValidationProblemsGroup(Composite parent) {
		...
		liveProblemDisplayField = new RadioGroupFieldEditor(IPreferenceConstants.VALIDATION_LIVE_PROBLEMS_DISPLAY,
				ValidationUIMessages.Validation_liveValidationDestinationPrompt, 1,
				new String[][] {
						{ ValidationUIMessages.Validation_liveValidationDestination_dialogComboItem,
								ValidationLiveProblemsDestination.DIALOG.getName() },
						{ ValidationUIMessages.Validation_liveValidationDestination_consoleComboItem,
								ValidationLiveProblemsDestination.CONSOLE.getName() } },
				outerComposite) {
		...
		warningsInDialog = new BooleanFieldEditor(IPreferenceConstants.VALIDATION_LIVE_WARNINGS_IN_DIALOG,
				ValidationUIMessages.Validation_liveValidationWarnDialogPrompt, innerComposite1) {
		...
		showOutputView = new BooleanFieldEditor(IPreferenceConstants.VALIDATION_LIVE_SHOW_CONSOLE,
				ValidationUIMessages.Validation_liveValidationShowConsolePrompt, innerComposite2);
		addField(showOutputView);
	}

	/**
	 * Sets the enablement of the "Show warnings in dialog ..." and "Show Output
	 * view ..." checkboxes according to whether the Output view is the destination
	 * of live validation problems.
	 */
	void setCheckboxesEnablement(String newSelection) {
		if (newSelection != null) {
			warningsInDialog.setEnabled(ValidationLiveProblemsDestination.DIALOG.getName().equals(newSelection),
					innerComposite1);

			showOutputView.setEnabled(ValidationLiveProblemsDestination.CONSOLE.getName().equals(newSelection)
					|| !warningsInDialog.getBooleanValue(), innerComposite2);
	 ...
	 *         page will not close and becomes annoying.
	 *         <P>
