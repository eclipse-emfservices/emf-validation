...
public class ConstraintRegistry {
	...
	public IConstraintDescriptor getDescriptor(String pluginId, String id) {
		return descriptors.get(XmlConstraintDescriptor.normalizedId(pluginId, id));
	}
	...
	public void register(IConstraintDescriptor descriptor) throws ConstraintExistsException {
		...
		if (registered) {
			broadcastConstraintChangeEvent(new ConstraintChangeEvent(descriptor, ConstraintChangeEventType.REGISTERED));
		}
	}
	...
	public void unregister(IConstraintDescriptor descriptor) {
		...
		if (unregistered) {
			broadcastConstraintChangeEvent(
					new ConstraintChangeEvent(descriptor, ConstraintChangeEventType.UNREGISTERED));
		}
	}
	...
	public synchronized void addConstraintListener(IConstraintListener listener) {
		if (indexOf(listener) < 0) {
			if (constraintListeners == null) {
				...
				IConstraintListener[] newListeners = new IConstraintListener[constraintListeners.length + 1];
	...
	public synchronized void removeConstraintListener(IConstraintListener listener) {
		...
		if (index >= 0) {
			...
			System.arraycopy(constraintListeners, index + 1, newListeners, index,
	...
	/**
	 * Broadcasts the specified <code>ConstraintChangeEvent</code> to all constraint
	 * listeners. This method is used internally by constraints to send
	 * notifications when they have changed.
	 * <p>
