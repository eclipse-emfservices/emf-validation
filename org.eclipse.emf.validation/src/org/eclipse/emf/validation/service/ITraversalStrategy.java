 ...
 * the relationships between these roots (e.g., containment), so it is up to the
 * strategy implementation to determine whether any optimization can be made in
...
public interface ITraversalStrategy {
	...
	/**
	 * An implementation of the {@link ITraversalStrategy} interface that is
	 * recursive: it iterates over the content trees of the roots, according to the
	 * basic EMF {@link EObject#eAllContents()} tree-iterator. One bit of value-add
	 * that it offers, though, is that it avoids duplication of sub-trees where one
	 * of the traversal roots is actually contained within another.
	 * <p>
	 * This traversal strategy assumes that only the root elements of the traversal
	 * may be in different client contexts, not any contents of their sub-trees.
	 * </p>
	 *
	 * @author Christian W. Damus (cdamus)
	 */
	 ...
	 * validated and a progress monitor to track the operation's progress. The
	 * receiver must initialize the <code>monitor</code> by invoking the
	 ...
	 * {@link IProgressMonitor#worked(int)} method. The validator will not make any
	 ...
	 *                       receiver may retain this progress monitor for the
	...
	/**
	 * Queries whether the next element in the traversal will <em>potentially</em>
	 * be in a different
	 * {@linkplain org.eclipse.emf.validation.internal.service.IClientContext client
	 * context} than the one before. The validation framework will then re-compute
	 * the current client context. If there is no next element, then this method's
	 ...
	 * <code>true</code>. This results in absolutely correct validation, but may
	 * hamper performance where assumptions can be made about the delineation of
	 * client contexts.
	 * </p>
	 * <p>
	 * A good rule-of-thumb for determining when the client context may change is as
	 * follows:
	 * <ul>
	 * <li>every traversal root defines a client context (see above)</li>
	 * <li>any object traversed by a non-containment reference may change the client
	 * context</li>
	 * <li>any object traversed by a containment relationship does not change the
	 * client context (it is in the same sub-tree), unless that element is an
	 * {@link org.eclipse.emf.ecore.EAnnotation}</li>
	 * <li><code>EAnnotation</code>s are commonly used to bridge between metamodels.
	 * It is probably a good idea to return <code>true</code> for annotations and
	 * their contents (as no assumption about the nature of the metamodel and its
	 * client applications can be made)</li>
	 * </ul>
	 ...
	 *                use this information to exclude or include portions of the
	 *                model based on the status of the <code>element</code>
	 */
