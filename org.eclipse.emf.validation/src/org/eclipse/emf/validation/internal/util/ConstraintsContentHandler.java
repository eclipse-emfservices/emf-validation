...
public class ConstraintsContentHandler extends DefaultHandler {
	...
	/**
	 * Processing instruction parameter indicating the version of the constraint
	 * markup language. The default (and currently only) version is 1.0.
	 */
	...
	/**
	 * Maintains a stack of XML elements as they are encountered by the SAX parser.
	 * Elements are popped onto and off of the stack as the SAX parser sees them
	 * come and go. Whenever an element is on the top of the stack, it can have more
	 * data added to it.
	 *
	 * @author Christian W. Damus (cdamus)
	 */
	private class Stack {
		private final List<XmlConfigurationElement> contents = new java.util.ArrayList<>();
		private final List<StringBuffer> bodies = new java.util.ArrayList<>();
		private int lastIndex = -1;

		/**
		 * Queries whether the stack is empty.
		 *
		 * @return <CODE>true</CODE> if the stack has no elements; <CODE>false</CODE>,
		 *         otherwise
		 */
		...
		/**
		 * Pops an element from the top of the stack. At this point, the body of the
		 * element which has been accumulating is assigned to it.
		 *
		 * @return the former top element.
		 * @throws SAXException if the stack is empty (in which case it cannot be
		 *                      popped)
		 */
		...
		/**
		 * Gets the string buffer which is accumulating the body of the current top
		 * element of the stack.
		 *
		 * @return the top element's body
		 * @throws SAXException if the stack is empty (in which case there is no body)
		 */
	...
	/**
	 * Helper class that caches resource bundles by host OSGI bundle and name.
	 *
	 * @author Christian W. Damus (cdamus)
	 */
	private static final class ResourceBundleCache {
		private final Map<Bundle, Map<String, ResourceBundle>> map = new java.util.HashMap<>();

		/**
		 * Obtains the resource bundle named <code>baseName</code> in the specified OSGI
		 * bundle's classpath, if it exists in this cache.
		 *
		 * @param osgiBundle an OSGI bundle that defines a classpath
		 * @param baseName   the base of the possibly locale-extended resource file name
		 * @return the cached resource bundle, or <code>null</code> if it does not exist
		 *         in the cache
		 */
		...
		/**
		 * Puts the resource bundle named <code>baseName</code>, in the specified OSGI
		 * bundle's classpath, into this cache.
		 *
		 * @param osgiBundle an OSGI bundle that defines a classpath
		 * @param baseName   the base of the possibly locale-extended resource file name
	...
	/**
	 * Initializes me with the extension which defines the XML that I parse. The
	 * extension is used to gain information about the source plug-in.
	 *
	 * @param extension the source extension
	 * @param baseUrl   base URL on which any relative URLs of referenced XML files
	 *                  are constructed
	 */
	...
	/**
	 * Obtains the configuration element that I have parsed. Should only be called
	...
	public void startElement(String namespaceURI, String localName, String qName, Attributes atts) {
		...
		Map<String, String> attMap = new java.util.HashMap<>();

		for (int i = 0; i < attCount; i++) {
			attMap.put(atts.getQName(i), localize(atts.getValue(i)));
		}

		stack.push(new XmlConfigurationElement(qName, attMap, extension, baseUrl));
	}

	/**
	 * Pops an element from the top of the stack. This is retained as my result if
	 * it was the last element, otherwise it is added to the new top as a child
	 * element.
	 *
	 * @see #getResult
	 */
	...
	/**
	 * Implements the handling of the <tt>&lt;?emf-validation?&gt;</tt> processing
	 * instruction for the constraints markup.
	 *
	 * @throws SAXException if a version is indicated in the processing instruction
	 ...
	 * processing instruction by finding the bundle for use in subsequent string
	 * translation.
	 *
	...
	private void handleNlInstruction(String resourceBundleName) {
		try {
			...
			if (resourceBundle == null) {
				InputStream input = findLocalizedResource(bundle, resourceBundleName);

				if (input != null) {
					resourceBundle = new PropertyResourceBundle(input);
					input.close();

					resourceBundleCache.put(bundle, resourceBundleName, resourceBundle);
				}
			}
		} catch (Exception e) {
			Trace.catching(getClass(), "handleNlInstruction", e); //$NON-NLS-1$

			Log.warningMessage(EMFModelValidationStatusCodes.XML_RESOURCE_BUNDLE_NOT_FOUND,
	 ...
	 * specified <code>baseName</code> and the current locale, for localization, and
	 ...
	 * @throws IOException if a URL is found that locates the resource file, but an
	 *                     exception occurs in opening that URL
	 */
	 ...
	 * constraint XML will be parsed, and the cache therefore is no longer needed.
	 */
	...
	private void handleVersionInstruction(String version) throws SAXException {
		if (!version.equals(EXPECTED_VERSION)) {
			SAXException e = new SAXException(EMFModelValidationPlugin
					.getMessage(EMFModelValidationStatusCodes.XML_WRONG_VERSION_MSG, new Object[] { version }));
	...
	/**
	 * Implements the <tt>%<i>key</i></tt> localization mechanism in the XML file
	 * that I am parsing. Uses my extension to look up the resource, unless a
	 * <tt>&lt;?emf-validation?&gt;</tt> processing instruction has indicated a
	 * different resource bundle.
	 *
	 * @param s the string to localize, if it starts with a % character
	 * @return the localized string, or just <code>s</code> if no localization was
	 *         needed
	 */
	protected String localize(String s) {
		if ((s == null) || !s.startsWith("%")) { //$NON-NLS-1$
			...
			return Platform.getResourceString(Platform.getBundle(extension.getNamespaceIdentifier()), s);
		} else {
	 ...
	 * @return the localized string, or just <code>s</code> if no localization was
	 *         needed or the resource was not found
	 */
	...
	public void fatalError(SAXParseException e) throws SAXException {
		Log.errorMessage(EMFModelValidationStatusCodes.ERROR_PARSING_XML_FILE,
				EMFModelValidationStatusCodes.ERROR_PARSING_XML_FILE_MSG, new Object[] { getFileName() }, e);
	...
	public void warning(SAXParseException e) throws SAXException {
		Log.warningMessage(EMFModelValidationStatusCodes.ERROR_PARSING_XML_FILE,
				EMFModelValidationStatusCodes.ERROR_PARSING_XML_FILE_MSG, new Object[] { getFileName() }, e);
