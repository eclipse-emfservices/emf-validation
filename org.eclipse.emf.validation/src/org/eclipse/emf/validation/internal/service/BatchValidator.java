...
public class BatchValidator extends AbstractValidator<EObject> implements IBatchValidator {
	 ...
	 * @param (output)          the set of client contexts to be updated with all of
	 *                          the encountered contexts while performing
	 *                          validation.
	 */
	private void validate(ITraversalStrategy traversal, List<IStatus> evaluationResults, AbstractValidationContext ctx,
			Collection<? extends EObject> objects, GetBatchConstraintsOperation operation,
			Set<IClientContext> clientContexts) {
		...
		return result.isEmpty() ? Status.OK_STATUS
				: (result.size() == 1) ? result.get(0)
						: new MultiStatus(EMFModelValidationPlugin.getPluginId(), 0,
								result.toArray(new IStatus[result.size()]), result.get(0).getMessage(), null);
	}
	...
	private static class DefaultRecursiveTraversalStrategy implements ITraversalStrategy {
		...
		public void startTraversal(Collection<? extends EObject> traversalRoots, IProgressMonitor monitor) {
			...
			for (Map.Entry<ITraversalStrategy, Collection<EObject>> next : delegates.entrySet()) {
				...
				SubProgressMonitor sub = new SubProgressMonitor(monitor, 1024,
