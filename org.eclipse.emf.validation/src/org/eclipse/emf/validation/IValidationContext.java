 ...
 * evaluated on which model element. Thus, the context will seem to know what
 * constraint implementation is calling it, but this is just a consequence of
 * the fact that constraints are invoked strictly one at a time by the context,
 * itself.
 * </p>
 * <p>
...
public interface IValidationContext {
	...
	/**
	 * Gets all of the {@link org.eclipse.emf.common.notify.Notification}s that are
	...
	/**
	 * In the case of a live constraint evaluation, obtains the particular feature
	 * on which the constraint is triggered. Note that this only applies to
	...
	/**
	 * <p>
	 * In the case of live constraint evaluation that is triggered by a particular
	 * feature, obtains the new value of the feature that is being validated. The
	 * exact contents of the result are as follows:
	 * <ul>
	 * <li>Add or Add Many: in the "Add" case, a single object added to the feature.
	 * In "Add Many" case, a collection of objects added</li>
	 * <li>Remove or Remove Many: in the "Remove" case, a single object removed from
	 * the feature. In "Remove Many" case, a collection of objects removed</li>
	 * <li>Set, Unset, or Move: the final value of the feature at the end of the
	 * transaction (i.e., its current value)</li>
	 * <li>Create: a single object or a collection of objects created in the
	 * feature</li>
	 * <li>Resolved: a single object or a collection of objects resolved in the
	 * feature</li>
	 * <li>Resolving Adapter: a single adapter or a collection of adapters removed
	 * from the adapter list of the {@link #getTarget() target}</li>
	 * <li>Others: the final value of the feature at the end of the transaction
	 * (i.e., its current value)</li>
	 * </ul>
	 * .
	 * </p>
	 * 
	 ...
	 * the specified <code>eObject</code> in the current validation operation.
	 * Constraints can use this facility to short-circuit processing, especially in
	 * a recursive batch validation, of objects that they have already checked in
	 * processing the current {@link #getTarget() target}.
	 * </p>
	 * <p>
	 * For example, checking for dependency cycles in a UML model requires
	 * traversing the dependency relationships from an element as far as they reach
	 * throughout the model. Every element thus traversed can subsequently be
	 * ignored by the constraint, as ground that needs not be trodden again, whether
	 * a cycle was found or not.
	 * </p>
	 * <p>
	...
	/**
	 * <p>
	 * Causes the current constraint to be disabled for the remainder of the Eclipse
	 * session, because it is no longer viable due to some run-time exception.
	 * </p>
	 * <p>
	 ...
	 * anything that the caller defines. The value is <code>null</code> until it is
	 * set by this same constraint using the {@link #putCurrentConstraintData}
	 * method. The working data persists only for the duration of the validation
	 * operation.
	 * </p>
	 * <p>
	 * The constraint may use this to cache any data that may be of use to it in
	...
	/**
	 * Puts into this context some working data that is to be cached for the current
	...
	/**
	 * Adds a result to the result locus of the current constraint. The result locus
