...
public final class EMFEventType implements Serializable {
	...
	public static final EMFEventType REMOVING_ADAPTER = new EMFEventType("Removing Adapter", //$NON-NLS-1$
			false, Notification.REMOVING_ADAPTER);

	/**
	 * A custom "Create" event, which is not implemented by EMF any longer but may
	 * be simulated by clients of the validation framework. (Corresponds to
	 * <code>0</code>).
	 */
	public static final EMFEventType CREATE = new EMFEventType("Create", //$NON-NLS-1$
			true, 0); // use the literal value from MSL because EMF deprecates it
	...
	/** All of my values. */
	private static final List<EMFEventType> predefinedInstances = Collections
	...
	private EMFEventType(String name, boolean featureSpecific, int notificationTypeCode) {
		...
		if (nameToInstance.containsKey(name)) {
			IllegalArgumentException e = new IllegalArgumentException("Duplicate event type name: " + name); //$NON-NLS-1$

			Trace.throwing(EMFEventType.class, "<init>", e); //$NON-NLS-1$

			EMFModelValidationPlugin.log(EMFModelValidationStatusCodes.DUPLICATE_EVENT_TYPE,
					NLS.bind(EMFModelValidationStatusCodes.DUPLICATE_EVENT_TYPE_MSG, name), e);
